...
# Get division questions from the b_division_questions.txt file
def read_b_div_q():  # read_basic_division_questions
...
def read_b_div_a():  # read_basic_division_answers
...
# Get multiplication questions from the b_multiplication_questions.txt file
def read_b_mul_q():  # read_basic_multiplication_questions
...
# Get multiplication answers from the b_multiplication_answers.txt file
def read_b_mul_a():  # read_basic_multiplication_answers
...
def read_b_sub_q():  # read_basic_subtraction_questions
...
def read_b_add_q():  # read_basic_addition_questions
...
def print_player_stats(player_level, player_level_progress, player_name):
    player_stats = ("|  Level: " + str(player_level) + "  |  " +
                    "Level progress: " + str(player_level_progress) + "/" +
                    str(10 + (int(player_level) * 2)) + "  |  " + "User: " +
                    player_name + "  |\n")
    print("_" * (len(player_stats) - 1))
    print(player_stats)


# Removes all the list features from the player info string
# and puts it back into a format
# that is displayed the same as the text document it grabs from
...
# Put the changed information/player_info
# back onto the txt file so it can be read again later.
...
def show_quiz_stats(correct_answers, player_level_progress):
    if (correct_answers >= 8):
        print("\nWell done, excellent work! You got " +
              str(correct_answers) + " questions correct.\n")
    elif (correct_answers >= 4 and correct_answers <= 7):
        print("\nGood job. You got " +
              str(correct_answers) + " questions correct.\n")
    elif (correct_answers >= 0 and correct_answers <= 3):
        print("\nYou got " + str(correct_answers) +
              " questions correct. Keep trying to improve your score.\n")
...
def achievements_function(player_level):
    ...
    if player_level >= 1:
        achievements[0] = 'Newbie: Start the program'
    if player_level >= 5:
...
# Welcome function
def welcome_user(player_name, player_info,
                 player_level, player_level_progress):
    if (player_name == "default"):
        ...
        print("This program is to help you learn basic addition, "
...
def replay_current_quiz(chosen_catagory, player_level_progress):
    ...
    if try_again == "y":
        if (chosen_catagory == 1):
            ...
            player_level_progress = show_quiz_stats(correct_answers,
                                                    player_level_progress)
            player_level_progress = replay_current_quiz(chosen_catagory,
                                                        player_level_progress)
...
def choose_catagory(player_level_progress):
    ...
    while type(chosen_catagory) != int:
        ...
        if (chosen_catagory == 1):
            ...
            print("\nYou chose division, "
                  "remember this symbol '/' means divide.\n")
            correct_answers = div_quiz()
            player_level_progress = show_quiz_stats(correct_answers,
                                                    player_level_progress)
        else:
...
def check_levelup(player_level, player_level_progress, achievements):
    ...
    if player_level_progress >= ((int(player_level) * 2) + 10):
        player_level_progress = (player_level_progress -
                                 ((int(player_level) * 2) + 10))
        player_level += 1
    player_level_progress = str(player_level_progress)
    player_level = str(player_level)
...
def return_values_player_info(player_level,
                              player_level_progress, player_info):
...
def main(achievements):
    (player_info, player_name, player_level,
     player_level_progress) = read_player_info()
    ...
    (player_level, player_level_progress,
     achievements) = check_levelup(player_level,
                                   player_level_progress, achievements)
    player_info = return_values_player_info(player_level,
                                            player_level_progress, player_info)
    info_to_file(player_info)
    main(achievements)
...
def __inti__():
    (player_info, player_name, player_level,
     player_level_progress) = read_player_info()
    player_name, player_info = welcome_user(player_name,
                                            player_info, player_level,
                                            player_level_progress)
    info_to_file(player_info)
    return player_info, player_name, player_level, player_level_progress
